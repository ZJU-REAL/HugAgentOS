...
| MCP 工具 | ✅ 7 个通用工具：联网搜索、网页抓取、图表生成、批量执行、自动化任务管理、技能管理、知识库检索 | ➕ 数仓查询（`query_database`）与产业知识中心插件（1 个 MCP、27 个工具、14 个 Skills） |
| 能力中心自助 | ✅ MCP 市场（Token / OAuth 安装）、自建私有 MCP（远程 HTTP/SSE）、私有技能（手写 / zip 上传），owner 隔离 | ➕ 组织级权限位治理、MCP/技能上架审核与蒸馏 |
...
| 系统管理台（/config） | ✅ 提示词管理：系统与桌面版本、预览、导入导出（需实例管理员权限） | ➕ 用户 / 团队 / 邀请 / 安全 / 服务配置 / License 面板 |
