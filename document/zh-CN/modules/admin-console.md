...
桌面双模式的执行位置按会话保存，普通聊天、计划生成与执行、自主循环及其续跑和取消均沿用同一执行端。选择本机后，模型仍可通过云端网关调用，但会话和任务日志保存在本机。浏览器打开云端 `/config` 时仅查询云端记录；查看本机任务请使用桌面端调用日志。运行端标签不表示模型的部署位置，也不会把本机对话自动上传到云端。
