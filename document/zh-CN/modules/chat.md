...
工具完成事件同时携带展示结果和对应的模型结果步骤，两者在同一次快照中保存。模型步骤的追加也独立触发落库；快照写入失败不会清除待保存状态，定时快照会重试。跨供应商或模型回放时，formatter 会核对思考的来源供应商、模型和协议；兼容思考保留原始结构及签名，不兼容或来源不明的可读思考转换为带明确标记的历史参考文本，不作为目标模型的原生推理状态。当前 Anthropic 模型的思考若缺失签名，会明确报错。
