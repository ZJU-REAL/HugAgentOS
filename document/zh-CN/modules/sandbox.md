...
**MySpace 实时对账**：写在 `/myspace` 下的文件**就是**用户「我的空间」里的文件，两侧任何时刻看到的都应是同一份状态。对账在三个时机发生（`core/llm/tools/myspace_mirror.py`）：

| 时机 | 方向 | 做什么 |
|---|---|---|
| 每次 `bash` 执行前 | 我的空间 → 沙箱 | 界面上的上传/改动落进镜像目录（bind mount 下即刻可见）；界面上删掉的文件同步从镜像移除 |
| 每次 `bash` 执行后 | 沙箱 → 我的空间 | 这条命令期间落盘的文件登记成 artifact |
| 打开「我的空间」列表时 | 双向 | 读时再对一次账，长命令跑到一半时用户看到的也是当下状态 |

反向登记按文件性质分两类，**不是一律推**：

- **新文件**（还没有登记记录）→ 直接登记展示，不打断用户；子智能体、批量、定时任务写的一样生效。内容已经落在用户空间的磁盘上，此时再拦只会造出"界面看不见却删不掉、还被下一个会话挂到"的隐形文件。
- **改动 / 删除用户已有的文件** → 主对话里过 `MYSPACE_WRITE_CONFIRM` 确认门，覆盖或删除既有文件要用户点头；**子智能体 / 批量 / 定时任务这类问不到人的场景直接生效，不拒绝** —— 拒绝拦不住已经发生的改动（文件在沙箱里早就改掉/删掉了），只会让界面停在旧状态、两边不一致。删除靠命令前后的镜像清单差集认出来（`rm` 不经过任何工具）。
- **用户已删除的文件绝不复活**：镜像里的残留副本一律不登记，反过来由正向对账清掉；只有删除之后沙箱又写了同名文件才算新内容。

其它要点：

- **不看命令文本、不看退出码**：路径可以由变量拼出、可以先 `cd` 再用相对路径，靠命令串里有没有 `myspace` 字样判断必然漏；命令失败前写出的文件同样要登记。
- **判据是执行前后的目录快照差集，不是时间窗口**：执行前记下 `{路径: (mtime, 大小)}`，执行后再列一次，"快照里没有"或"变了"的就是这条命令动过的。真机实测发现 Linux 写文件记的 mtime 取自内核**粗粒度时钟**，比 `time.time()` 慢 6~10 毫秒——纯按"命令开始时刻"做时间窗口，会把命令刚写出来的文件判成窗口之前的旧文件而漏掉。拿目录自己和自己比不受时钟影响，长命令短命令一视同仁。
- **判据是执行前后的目录快照差集，不是时间窗口**：执行前记下 `{路径: (mtime, 大小)}`，执行后再列一次，"快照里没有"或"变了"的就是这条命令动过的。真机实测发现 Linux 写文件记的 mtime 取自内核**粗粒度时钟**，比 `time.time()` 慢 6~10 毫秒 —— 纯按"命令开始时刻"做时间窗口，会把命令刚写出来的文件判成窗口之前的旧文件而漏掉。拿目录自己和自己比就不受时钟影响，长命令短命令一视同仁。
...
- 一条命令只产出一个文件时才把它钉进对话产物区；批量作业一次落几十个文件，逐个钉卡会淹掉产物区，它们在「我的空间」里照样可见可下载。

历史欠账（修复上线前积压的未登记文件 / 该删没删的残留）用 `scripts/reconcile_myspace_mirror.py` 一次性对齐（`--dry-run` 先看规模）。
