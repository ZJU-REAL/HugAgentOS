...
而不是直接进入工作台。向导依次设置界面语言、模型、互联网搜索、文件解析、永久记忆
和本体核验。

- 模型步骤包含必填的主对话模型，以及可选的索引（embedding）模型和重排
  （reranker）模型。保存时后端会实测所有已填写模型的连通性，并分别指派对话、
  `embedding` 和 `reranker` 角色；没有已启用的 `main_agent` 模型时，完成接口会拒绝
  解锁工作台。
- 配置并指派索引模型后，后续步骤会立即重新检测依赖并开放永久记忆开关；重排模型
  用于提高长期记忆与知识库检索结果的相关性。两个检索模型留空均不阻塞初始化。
- 互联网搜索和外部文件解析均可跳过，以后可在**设置 → 系统管理**中补充。
- 记忆和本体开关会先检查实例是否具备对应服务或已发布领域包；不可用时保持关闭，
  不会制造“已开启但运行时无效”的状态。
- CE 新部署在浏览器没有保存过偏好时默认开启**显示调度过程**；用户在**设置 →
  会话设置**中关闭后会保留该选择。
...
| `GET /v1/me/teams` 等 | `edition_ee/routes/me_teams.py` | 用户侧团队查看、邀请成员、移除 / 退出（商业版 EE；CE 树物理不包含该模块） |
| `GET /v1/me/users/search` | `edition_ee/routes/me_teams.py` | 邀请成员时的用户搜索 |
...
| 实现文件 | 职责 |
|---|---|
| `edition_ee/auth/team_permissions.py` | 团队文件夹权限解析（商业版 EE） |
...
`edition_ee/auth/roles.py` 定义三级团队角色：`owner`（所有者）> `admin`（管理员）> `member`（成员）。团队文件权限映射在 `edition_ee/auth/team_permissions.py`：
...
按用户粒度的功能开关存储在 `users_shadow.metadata`（ORM 字段 `extra_data`）JSON 列中，由 Config 管理台的 EE 用户管理模块（`edition_ee/routes/config_users.py`）设置。**默认全部关闭**（关闭即从 metadata 移除键）：
...
- **团队管理**（`edition_ee/routes/config_teams.py`）：团队 CRUD、成员增删、角色设置（owner/admin/member）。
- **注册码管理**（`edition_ee/routes/config_invites.py`）：批量生成、列表、吊销、删除。注册码形如 `JX-ABCD-2345`（`edition_ee/auth/invite.py`，字符表去除易混淆的 O/0、I/1 等），默认有效期 `INVITE_CODE_DEFAULT_TTL_HOURS`（168 小时）；消费用条件 UPDATE 保证并发安全，可预绑定团队与角色。
- **用户侧**（`edition_ee/routes/me_teams.py`）：团队 owner/admin 可直接邀请成员、移除成员；成员可退出。
...
| 团队角色 / 文件权限（EE） | `src/backend/edition_ee/auth/roles.py`、`src/backend/edition_ee/auth/team_permissions.py` |
| 项目 / 会话分享权限（EE） | `src/backend/edition_ee/auth/project_permissions.py`、`src/backend/edition_ee/auth/chat_share_permissions.py` |
| 管理凭证依赖 | `src/backend/api/deps.py` |
| 用户资料 / 偏好 | `src/backend/api/routes/v1/users.py` |
| 用户权限位管理 | `src/backend/edition_ee/routes/config_users.py` |
...
| 邀请码 | `src/backend/edition_ee/auth/invite.py`、`src/backend/edition_ee/routes/config_invites.py` |
| 团队管理 | `src/backend/edition_ee/routes/config_teams.py`、`src/backend/edition_ee/routes/me_teams.py` |
| License 能力位守卫 | `src/backend/edition_ee/licensing/features.py`、`src/backend/edition_ee/licensing/deps.py` |
