...
`LOCAL_AUTH_ENABLED=true`（默认开启）时，`api/routes/v1/mock_sso.py` 中的 `login_router` 提供统一登录页：

- `GET /login` — 渲染登录 / 注册双 Tab 页面（注册 Tab 仅在本地账号开启时显示）
- `POST /login` — 账号密码登录提交
- `POST /register` — 本地账号注册（可要求注册码，见下文「邀请码」）

密码哈希在 `core/auth/password.py`，最小长度由 `PASSWORD_MIN_LENGTH`（默认 8）控制。本地账号存储于 `local_users` 表（`core/db/models` 的 `LocalUser`），与影子用户表一对一关联。
