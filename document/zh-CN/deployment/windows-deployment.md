...
> 最后更新：2026-07-21 ｜ [English](../../en/deployment/windows-deployment.md) ｜ 返回 [部署指南](README.md)
...
社区版边界检查的 CE 服务资源；首次启动时在当前用户目录创建独立 Python 环境，不要求
Docker Desktop、WSL2、PostgreSQL 或 Redis。
...
3. 启动桌面客户端，等待服务设置页完成资源复制、依赖安装和健康检查。
...
`data`。卸载桌面客户端默认也保留此目录，避免误删数据；确认不再使用后可手动删除。
