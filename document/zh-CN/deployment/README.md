...
在云端网页的侧栏帮助菜单点击「下载客户端」，网页会识别 Windows、macOS 或 Linux，
读取服务器为该平台发布的最新版本并开始下载。下载入口以分平台桌面发布清单为准；
尚未发布该平台客户端时会提示暂无可用版本，不会转到其他系统的安装包或历史文件。
实际下载格式取决于该平台发布的产物（例如 Windows `.exe`、macOS `.app.tar.gz`、
Linux `.deb` 或 `.AppImage`）。macOS 的 `.app.tar.gz` 解压后得到应用程序。
