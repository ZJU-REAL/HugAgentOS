...
默认工作区按对话持久保存于 `<数据目录>/workspace/.sessions/<会话哈希>/`。
同一对话重用目录，不因空闲自动删除；不同对话相互分开。
文件工具与 Bash 使用同一相对路径基准，绝对路径直接指向本机文件，不套用容器路径别名。
绑定本地项目后，使用项目上下文提供的真实文件夹路径。

并发命令各用独立临时脚本，保持相同会话工作目录，完成后仅清理自己的脚本。
技能列表提供本次运行所用版本的真实说明文件路径，技能不指定固定部署目录；
脚本以正确引用的真实路径启动，产物留在工作目录。Windows 盘符、中文和空格路径均按命令参数处理。
容器沙箱继续使用独立的挂载及 My Space 规则。

## 本机文件交付

本机模式使用 `pin_to_workspace(file_paths=[...])` 直接交付当前会话工作目录或绑定项目内的文件，
不再向模型注册 `sandbox_get_artifact`。项目文件引用原文件，会话工作目录产物由交付工具自动
保存为附件；读取前仍执行本机权限检查。建议传真实绝对路径；绑定项目时交付工具的相对路径
以项目目录为准，未绑定时以当前会话工作目录为准。云端仍使用登记文件后按 ID 交付的流程。
全部失败时返回 `ok: false`，部分失败时成功列表与失败列表分别列明。
