...
const tauriConfig = JSON.parse(readFileSync(join(desktopDir, 'src-tauri', 'tauri.conf.json'), 'utf8'));
...
function findPython() {
  ...
  throw new Error('Python 3.11 or later is required to generate the bundled CE server.');
}

console.log('[desktop] Building the desktop web application');
run(npmCommand, [...npmPrefix, 'run', 'build'], { cwd: join(repoRoot, 'src', 'frontend') });

const python = findPython();
const ceArgs = [
  ...python.prefix,
  join(repoRoot, 'scripts', 'build_ce.py'),
  '--out',
  generatedRoot,
];
if (!process.env.CI && process.env.HUGAGENT_RELEASE_BUILD !== '1') ceArgs.push('--allow-dirty');

console.log('[desktop] Generating the CE server payload');
...
run(npmCommand, [...npmPrefix, 'install', '--no-audit', '--no-fund', '--no-package-lock'], {
...
if (revision.status !== 0) throw new Error('Unable to resolve the Git revision for the bundle.');
