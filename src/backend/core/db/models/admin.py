...
class AdminSkill(Base):
    ...
    user_intro    = Column(Text, nullable=True)            # user-facing intro shown in the capability center (Markdown)
    ...
    dependencies  = Column(JSONType, default=dict)             # {"pip":[...], "npm":[...], "apt":[...], "warnings":[...]}
...
class SandboxRebuild(Base):
    """Tracks an admin-triggered rebuild of sandbox images (script-runner / opensandbox).

    Used to apply aggregated skill dependencies. Each row represents one
    asyncio-backed background job; status drives the admin progress UI.
    """
    __tablename__ = "sandbox_rebuilds"

    run_id        = Column(String(64), primary_key=True)
    status        = Column(String(20), nullable=False, default="pending")
    targets       = Column(JSONType, default=list)          # ["script-runner", "opensandbox"]
    deps_hash     = Column(String(64))                       # SHA256 of aggregated manifests at trigger time
    started_at    = Column(TIMESTAMP(timezone=True), default=datetime.utcnow, nullable=False)
    completed_at  = Column(TIMESTAMP(timezone=True))
    log           = Column(Text, default="")                 # docker compose stdout/stderr (truncated to 256 KB)
    ...
    __table_args__ = (
        CheckConstraint(
            ...
            name="sandbox_rebuilds_status_check",
        ),
        Index("idx_sandbox_rebuilds_started_at", "started_at"),
        Index("idx_sandbox_rebuilds_status", "status"),
    )
...
class AdminSkillDraft(Base):
    """Skill distillation draft — a candidate skill automatically distilled from a successful trajectory, pending admin review."""
    __tablename__ = "admin_skill_drafts"

    draft_id              = Column(String(64), primary_key=True)
    proposed_skill_id     = Column(String(128), nullable=False)
    decision              = Column(String(16), nullable=False)  # new_skill | patch
    patch_target_id       = Column(String(128))
    ...
    extra_files           = Column(JSONType, default=dict)

    source_chat_id        = Column(String(64), nullable=False)
    source_user_id        = Column(String(64), nullable=False)
    source_trace_ids      = Column(JSONType, default=list)
    trajectory_digest     = Column(Text)
    distillation_cost_usd = Column(Numeric(8, 4))
    # auto = nightly single-session auto distillation; colleague = background persona-level colleague distillation (mirrored from persona_distill_jobs)
    draft_kind            = Column(String(16), nullable=False, default="auto")
    source_job_id         = Column(String(64))                 # colleague drafts link back to persona_distill_jobs.job_id
    ...
    __table_args__ = (
        ...
        Index("idx_admin_skill_drafts_status", "review_status", "created_at"),
        Index("idx_admin_skill_drafts_source", "source_chat_id"),
    )
...
class PluginMarketPackage(Base):
    ...
    __table_args__ = (
        Index("idx_plugin_market_packages_category", "category"),
    )
...
class MarketplaceListingState(Base):
    ...
    ``visibility``: visibility scope. ``public`` (default, missing row is
    synonymous) = visible to everyone; ``scoped`` = visible only to
    users/teams/roles granted in ``marketplace_visibility_grants``. Governs
    only marketplace browsing and installation; does not retroactively
    affect installed instances.
    """
    ...
    visibility = Column(String(16), nullable=False, default="public")  # public | scoped
    updated_at = Column(TIMESTAMP(timezone=True), default=datetime.utcnow, onupdate=datetime.utcnow)
    updated_by = Column(String(64))


class MarketplaceVisibilityGrant(Base):
    """Marketplace item visibility-scope grant (the whitelist when ``visibility='scoped'``).

    ``kind``/``item_id`` mirror ``MarketplaceListingState``;
    ``principal_type`` = ``user`` | ``team`` | ``role``, and matching any of
    the three principal types grants visibility (union, no precedence).
    EE-only table (configured server-side via the admin marketplace routes);
    CE has no admin console and is always visible to everyone — the resolver
    wraps queries against this table in try/except as a fallback (see
    core/auth/marketplace_visibility.py).
    """
    __tablename__ = "marketplace_visibility_grants"

    kind           = Column(String(16), primary_key=True)   # plugin | skill | agent
    item_id        = Column(String(160), primary_key=True)
    principal_type = Column(String(16), primary_key=True)   # user | team | role
    ...
    __table_args__ = (
        CheckConstraint(
            "principal_type IN ('user', 'team', 'role')",
            name="marketplace_visibility_grants_principal_type_check",
        ),
        Index("idx_mkt_vis_grants_principal", "principal_type", "principal_id"),
    )
