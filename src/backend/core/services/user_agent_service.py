...
class UserAgentService:
    ...
    def create(
        ...
        if owner_type == "user":
            ...
            if not team_id:
                raise ValueError("team_id required for team agents")
            # Only the team owner/admin may create team sub-agents
            if not self._is_team_manager(user_id, team_id):
                raise PermissionError("Only team owner/admin can create team agents")
            count = self.repo.count_team_agents(team_id)
            if count >= MAX_TEAM_AGENTS:
                raise ValueError(f"Maximum {MAX_TEAM_AGENTS} agents per team reached")
        ...
        record = {
            ...
            "team_id": team_id if owner_type == "team" else None,
            "created_by": user_id,
            **data,
    ...
    def _team_role(self, user_id: Optional[str], team_id: Optional[str]) -> Optional[str]:
        """The current user's role in a team (owner/admin/member); None for non-members."""
        if not user_id or not team_id:
            return None
        from core.db.repository import TeamRepository

        return TeamRepository(self.db).get_member_role(team_id, user_id)

    def _is_team_manager(self, user_id: Optional[str], team_id: Optional[str]) -> bool:
        return self._team_role(user_id, team_id) in ("owner", "admin")
    ...
    def _is_accessible(self, agent: UserAgent, user_id: str) -> bool:
        ...
        if agent.owner_type == "user" and agent.user_id == user_id:
            return True
        if agent.owner_type == "team" and agent.team_id:
            role = self._team_role(user_id, agent.team_id)
            if role is None:
                return False
            # Enabled team agents are visible to all members; disabled ones only to owner/admin (for management)
            return bool(agent.is_enabled) or role in ("owner", "admin")
        return False
    ...
    def _check_ownership(self, agent: UserAgent, user_id: Optional[str], owner_type: str) -> None:
        ...
        if agent.owner_type == "user" and agent.user_id == user_id:
            return
        if agent.owner_type == "team" and self._is_team_manager(user_id, agent.team_id):
            return
        raise PermissionError("You can only modify your own or your team's agents")
