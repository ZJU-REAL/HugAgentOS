...
def register_run_job(
    ...
    *,
    ...
    async def run_job(
        ...
        - **前台等还是后台跑，按作业规模自己判断；但任何情况下都别轮询**。默认
          ``wait=False`` 后台跑：工具立刻返回 job_id，此时**先把当前这轮回复收掉**——
          告诉用户作业已在后台开始、进度看输入框上方的状态条、随时可以继续聊别的。
          作业跑完（以及每隔一段时间）系统会**自动唤醒本会话**让你播报，不需要你守着。
          估算量级（工作项条数 × 每项要不要过模型 × 并发度）：一两分钟内能收的小作业用
          ``wait=True`` 原地等更顺手，一轮之内直接交付；几分钟以上就走默认的后台，
          长时间阻塞会让用户看不到中间进度、插不上话、也没法中途改主意；拿不准按后台走。
          无论哪种，反复调 ``action="status"`` 干等都纯属浪费轮次。
        ...
        Args:
            ...
            wait (`bool`): **默认 false = 后台跑**：立即返回 job_id，作业跑完 / 每隔一段
                ...
                true 则原地阻塞到作业结束，适合一两分钟内能收的小作业（省掉唤醒那一圈，
                一轮之内直接交付）；几分钟以上的作业别用，阻塞会让会话看起来「卡死在
                前台」，既看不到进度也没法中途干预。拿不准就用默认值。
