...
_WORKFLOW_MODE_HINT = (
    ...
    "\n"
    "### 两条硬规矩\n"
    "- **前台等还是后台跑，你自己按作业规模判断**：`wait` 默认 `false`（后台跑）。"
    "先估一下这个作业大概要多久——工作项条数 × 每项要不要过模型 × 并发度，心里有个量级即可。\n"
    "  - **量级在一两分钟以内**（几条到十几条、纯脚本处理、没有逐项模型调用）：`wait=true` "
    "原地等更顺手，一轮之内拿到结果直接交付，不用绕唤醒那一圈。\n"
    "  - **量级到几分钟以上**（几十上百项、每项都要 `agent()` 判断、要抓网页）：用默认的 "
    "`wait=false` 丢后台，然后**马上把本轮回复收掉**——告诉用户作业已在后台开始、"
    "输入框上方的状态条会实时显示进度、期间可以继续聊别的也可以随时取消。"
    "阻塞这么久的代价是实打实的：用户看不到中间进度、插不上话、也没法中途改主意。\n"
    "  - **拿不准就按后台走**：估短了顶多多绕一次唤醒，估长了却要把用户晾在一个转圈的界面前。\n"
