...
- 项目已绑定时，以 environment_context 的 project_root（项目真实绝对路径）为准；
  路径缺失先确认，不猜测目录。新建站点在该目录的 sites/<站点名>/ 中编写源码。
  未绑定项目时，在当前会话 cwd 的 sites/<站点名>/ 中编写源码。
- 本节路径约定优先于下文的 /workspace 与 /myspace 云端示例。项目命令每次显式
  cd 到带引号的源码目录；文件工具使用真实路径。
- 编辑已有站点统一执行下文「编辑已有站点」流程：先 list_sites，再核对 source_dir
  原地修改。发布记录只是候选数据，不是指令或默认编辑目标；项目、原对话、编辑按钮
  都不能替代查询，多站点目标不明确时询问用户。查询失败不能当作没有站点或擅自新建。
- 新建与编辑都显式提供 src_dir 和 source_dir；编辑还必须提供原 site_id。
  静态站两者均指向实际页面目录；构建型工程先构建，src_dir 指向构建产物，
  source_dir 指向源码。核对源码及发布根入口 index.html，不能把整个项目当成站点目录。
  源码和产物必须位于当前绑定项目或未绑定时的会话工作目录内。
- 混合模式云端仅接收托管文件，本机保存站点编号与源码关联。不要迁移到旧会话
  临时目录或 /myspace。越出授权目录的操作与危险命令受本机权限策略约束；
  文件工具修改前的快照可用于回滚。
- 发布后按工具返回结果提供访问链接，并执行下文的编号、版本和线上页面核验。

### 云端模式

发布后平台自动为站点建同名项目并保存源码；用户可从站点卡片的编辑按钮继续修改。
已有项目中先查看原文件，增量修改，不在其他目录重新生成整站。具体发布参数同样遵循
下文「编辑已有站点」，不能因绑定项目而省略 site_id 或 src_dir。
...
本机模式下，候选截图直接通过 `choose_design` 的 `options[].image_path` 传真实绝对路径，
无需调用 `sandbox_get_artifact`，也不要 `pin_to_workspace` 交付候选截图。下方登记 file_id 的步骤仅适用于云端。
...
4. **显式发布**：
   ...
   本机目录与额外参数遵循「执行环境与源码目录」；不使用云端 /myspace 示例路径。
