...
/** 「这台机器上装了什么」——四类能力落在本机的接口前缀，每项对应后端一个路由。
 *  新增一个能力入口时在这里加一行，判定逻辑本身不用动。 */
const LOCAL_CAPABILITY_PREFIXES = [
  'catalog', //          四类能力的启停目录（catalog.py）
  'plugins', //          插件安装 / 卸载 / 启停（plugins.py）
...
function underPrefix(path: string, prefixes: string[]): boolean {
  return prefixes.some((prefix) => path === `/v1/${prefix}` || path.startsWith(`/v1/${prefix}/`));
}
 ...
 *  个真源，是本机那份还不存在。
 *
 *  与云端市场打交道的请求（安装、申请上架、OAuth 授权）不在这张表里：包、审核流程和授权
 *  回调都只存在于云端，打到本机只会找不到对应条目。从市场装完之后，云端能力变更号会变，
 *  左侧边栏出现同步入口，用户点一下才把它落到本机。
 *
...
function capabilityTargetHeaders(url: string): Record<string, string> {
  ...
  if (underPrefix(path, CLOUD_ONLY_PREFIXES) || !underPrefix(path, LOCAL_CAPABILITY_PREFIXES)) {
    return {};
  }
  return localHeader();
}
