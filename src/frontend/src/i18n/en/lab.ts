...
export const LAB_DICT: Record<string, string> = {
  ...
  '本机服务尚未就绪，请稍后重试': 'The local service is not ready. Please try again shortly.',
  '执行位置': 'Execution location',
  '在当前电脑执行；执行时电脑需开机且本机服务运行': 'Runs on this computer; keep it on with the local service running.',
  '电脑关闭后仍可执行；无法直接访问本机文件': 'Runs even when this computer is off; cannot directly access local files.',
  '关联项目': 'Linked project',
  '选择任务需要访问的项目': 'Select the project this task needs to access',
  '请选择时区': 'Select a time zone',
  '项目与执行位置不匹配，本机目录必须在本机执行': 'The project does not match the execution location. Local directories require local execution.',
  '云端任务不能直接访问本机目录，请选择本机执行并绑定项目': 'Cloud tasks cannot access local directories. Choose local execution and link a project.',
  '读取本机目录的定时任务必须绑定本机项目': 'Tasks that read local directories must be linked to a local project.',
};
