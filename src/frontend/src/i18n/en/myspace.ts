...
export const MYSPACE_DICT: Record<string, string> = {
  ...
  '文件夹路径非法或超过 8 级': 'Invalid folder path or more than 8 levels',
  '无法读取拖入的文件夹': 'Cannot read the dropped folder',
  '登录身份或服务地址已改变，请重新选择文件': 'Account or server changed. Select files again.',
  '请选择原上传文件，或先清除上传记录': 'Select the original files, or clear the upload record first.',
  '上传清单包含重复路径': 'The upload contains duplicate paths.',
  '服务器未返回完整目录，请重试': 'The server returned an incomplete directory map. Retry the task.',
  '读取文件夹': 'Reading folder',
  '创建目录': 'Creating folders',
  '请求繁忙，等待后自动继续': 'Server busy. Waiting to resume automatically.',
  '部分上传未完成': 'Some uploads are incomplete',
  '请重新选择原文件夹以继续上传': 'Select the original folder again to resume',
  '上传进度：{done}/{total}，失败 {failed}': 'Uploaded {done}/{total}; failed {failed}',
  '无法保存上传记录，请保持页面开启': 'Cannot save upload progress. Keep this page open.',
  '查看失败文件': 'View failed files',
  '重试未完成项': 'Retry incomplete items',
  '清除上传记录': 'Clear upload record',
