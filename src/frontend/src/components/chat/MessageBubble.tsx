...
export const MessageBubble = memo(function MessageBubble({ m, messageIndex, currentChatId, send, exportChatRecord, regenerate, editAndResend }: MessageBubbleProps) {
  ...
  /** 这条消息发出时引用了哪几段会话。与追问引用同一位置呈现，只给标题，点开无副作用。 */
  const renderReferencedChats = () => {
    if (m.role !== 'user' || !m.referencedChats?.length) return null;
    return (
      <div className="jx-userRefs">
        <span className="jx-userQuoteLabel">{t('引用会话')}</span>
        {m.referencedChats.map((chat) => (
          <span
            key={chat.chat_id}
            className="jx-userRefChip"
            title={chat.overview || chat.title}
          >
            {chat.title}
          </span>
        ))}
                  ...
                  return (
                    <React.Fragment key={segKey}>
                      <div className={`jx-bubble ${m.role === 'user' ? 'user' : ''} ${m.isMarkdown ? 'jx-md' : ''} ${m.isStreaming && isLastSeg ? 'streaming' : ''}`}>
                        ...
                        {segIdx === 0 && renderReferencedChats()}
