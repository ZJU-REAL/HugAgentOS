...
- 完整包可由发行品牌预设首启模式；固定双模式的发行版直接进入一键初始化，不再重复选择模式。
...
初始化页与现有桌面端使用一致的光晕、轨道和进度高光动画。安装过程只展示当前阶段、
百分比和可操作的错误信息，不在页面底部暴露原始日志；完整日志仍保留在本机服务日志目录，
供排障使用。可用 `JX_DEFAULT_PROVISION_MODE=local_only|cloud_only|dual` 显式覆盖发行默认值。
